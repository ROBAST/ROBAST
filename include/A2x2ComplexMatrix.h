...
class A2x2ComplexMatrix {
 ...
 public:
  ...
  friend A2x2ComplexMatrix operator*(const std::complex<Double_t>& lhs,
    ...
    return A2x2ComplexMatrix(lhs * rhs.fC[0], lhs * rhs.fC[1],
                             lhs * rhs.fC[2], lhs * rhs.fC[3]);
  }
  friend A2x2ComplexMatrix operator*(const A2x2ComplexMatrix& lhs,
                                     const std::complex<Double_t>& rhs) {
    return A2x2ComplexMatrix(lhs.fC[0] * rhs, lhs.fC[1] * rhs,
                             lhs.fC[2] * rhs, lhs.fC[3] * rhs);
  }
  friend A2x2ComplexMatrix operator/(const A2x2ComplexMatrix& lhs, const std::complex<Double_t>& rhs) {
    return A2x2ComplexMatrix(lhs.fC[0] / rhs, lhs.fC[1] / rhs,
                             lhs.fC[2] / rhs, lhs.fC[3] / rhs);
  }
};
...
A2x2ComplexMatrix A2x2ComplexMatrix::operator*(const A2x2ComplexMatrix& other)
{
  ...
  return A2x2ComplexMatrix(A[0] * B[0] + A[1] * B[2],
                           A[0] * B[1] + A[1] * B[3],
                           A[2] * B[0] + A[3] * B[2],
                           A[2] * B[1] + A[3] * B[3]);
}
