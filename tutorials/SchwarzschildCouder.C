...
void SchwarzschildCouder() {
  ...
  const Double_t kZi[6] = {
      TMath::Power(kFp, -1) * 0.25,  // Zi
      TMath::Power(kFp, -3) * -0.189377, TMath::Power(kFp, -5) * -0.604706,
      TMath::Power(kFp, -7) * -4.21374,  TMath::Power(kFp, -9) * 21.8275,
      TMath::Power(kFp, -11) * -425.160};
  const Double_t kVi[6] = {
      TMath::Power(kFs, -1) * 0.25,  // Vi
      TMath::Power(kFs, -3) * 0.013625, TMath::Power(kFs, -5) * -0.010453,
      TMath::Power(kFs, -7) * 0.014241, TMath::Power(kFs, -9) * -0.012213,
      TMath::Power(kFs, -11) * 0.005184};
  const Double_t kYi[1] = {TMath::Power(kFf, -1) * 0.25};  // Yi

  // Make the primary mirror
  AGeoAsphericDisk* primaryV = new AGeoAsphericDisk(
      "primaryV", -1 * um, 0, 0 * m, 0, kDp / 2.,
      kDpinner / 2.);  // curvatures are set to infinity (0 = 1./inf)
  ...
  AGeoAsphericDisk* secondaryV = new AGeoAsphericDisk(
      "secondaryV", kZs, 0, kZs + 1 * um, 0, kDs / 2.,
      0 * m);  // curvatures are set to infinity (0 = 1./inf)
  ...
  AGeoAsphericDisk* focalV =
      new AGeoAsphericDisk("focalV", kZf - 1 * mm, 0, kZf, 0, 10 * cm * 7,
                           0.);  // curvatures are set to infinity (0 = 1./inf)
  ...
  AGeoAsphericDisk* focalObsV =
      new AGeoAsphericDisk("focalObsV", focalV->CalcF1(10 * cm * 7) - 1 * cm, 0,
                           focalV->CalcF1(10 * cm * 7), 0, 10 * cm * 7.3, 0.);
  ...
  graRMS->SetTitle(
      ";Field angle (deg);2 #times max{RMS_{sagital}, RMS_{tangential}} "
      "(arcmin)");
  TGraph* graT = new TGraph;
  graT->SetTitle(
      ";Field angle (deg);Photon propagation time spread (RMS) (ns)");

  for (Int_t n = 0; n < kN; n++) {
    hist[n] = new TH2D(Form("hist%d", n),
                       Form("#it{#theta} = %d (deg);X (arcmin);Y (arcmin)", n),
                       1000, -20, 20, 1000, -20, 20);
    histT[n] = new TH1D(
        Form("histT%d", n),
        Form("#it{#theta} = %d (deg);Propagation delay (ns);Entries", n), 120,
        -6, 6);

    Double_t deg = n;

    TGeoTranslation* raytr = new TGeoTranslation(
        "raytr", -1.2 * kZs * TMath::Sin(deg * TMath::DegToRad()), 0,
        1.2 * kZs * TMath::Cos(deg * TMath::DegToRad()));
    ...
    for (Int_t j = 0; j <= focused->GetLast(); j++) {
      ...
      */
      ...
      histT[n]->Fill((p[3] - (3.2 * kZs - kZf) / (TMath::C() * m)) /
                     1e-9);  // ns
    }                        // j
