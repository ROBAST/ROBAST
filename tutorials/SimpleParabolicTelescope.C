...
AOpticsManager* MakeGeometry() {
  ...
  TGeoTranslation* focal_tr =
      new TGeoTranslation("focal_tr", 0, 0, kFocalLength + 10 * um);
  ...
  TGeoTube* obs_tube1 =
      new TGeoTube("obs_tube1", 0, kFocalRad + 10 * um, 10 * um);
  TGeoTranslation* obs_tr1 =
      new TGeoTranslation("obs_tr1", 0, 0, kFocalLength + 30 * um);
  ...
  TGeoTube* obs_tube2 =
      new TGeoTube("obs_tube2", kFocalRad, kFocalRad + 10 * um, 10 * um);
  TGeoTranslation* obs_tr2 =
      new TGeoTranslation("obs_tr2", 0, 0, kFocalLength + 10 * um);
...
void SimpleParabolicTelescope() {
  ...
  for (Int_t i = 0; i < kN; i++) {
    ...
    hist[i] = new TH2D(Form("hist%d", i),
                       Form("#it{#theta} = %.1f (deg);X (cm);Y (cm)", deg), 300,
                       -3, 3, 300, -3, 3);

    TGeoTranslation* raytr =
        new TGeoTranslation("raytr", -kFocalLength * 2 * TMath::Sin(rad), 0,
                            kFocalLength * 2 * TMath::Cos(rad));
    TVector3 dir;
    ...
    for (Int_t j = 0; j <= focused->GetLast(); j++) {
      ...
      if (((i == 0) || (i == kN - 1)) &&
          (TMath::Abs(first[0]) < 1 * cm || TMath::Abs(first[1]) < 1 * cm)) {
