...
void CORSIKA() {
  ...
  std::cout << "Date of run: " << runheader->GetDateOfBeginRun().AsString()
            << std::endl;
  std::cout << "CORSIKA version: " << runheader->GetVersionOfProgram()
            << std::endl;
  ...
  for (int i = 1; i < nObsLevels + 1; i++) {
    std::cout << "Observation Level: " << runheader->GetHeightOfLevel(i) / m
              << std::endl;
  }  // i
  ...
  ARayArray* array =
      f.GetRayArray(telescope_number, array_number, zoffset, refractive_index);

  manager->TraceNonSequential(array);
  TObjArray* focused = array->GetFocused();
  TH2D* h =
      new TH2D("h", ";Camera X (cm);Camera Y (cm)", 200, -50, 50, 200, -50, 50);
