...
const double kMirrorT = 0.1 * mm;  // mirror thickness, intentionally use a very
                                   // thin thickness to avoid unnecessary
...
void DaviesCotton() {
  ...
  AOpticsManager* manager =
      new AOpticsManager("manager", "Davies-Cotton System");
...
void AddMirrors(AOpticalComponent* opt) {
  ...
  TGeoCompositeShape* mirComposite =
      new TGeoCompositeShape("mirComposite", "mirSphere:transZ*mirCut");
  ...
  double x[kNMirror] = {
      0,         0,         0,         0,         0,         0,
      0,         0,         1.5 * dx,  1.5 * dx,  1.5 * dx,  1.5 * dx,
      1.5 * dx,  1.5 * dx,  1.5 * dx,  1.5 * dx,  1.5 * dx,  1.5 * dx,
      -1.5 * dx, -1.5 * dx, -1.5 * dx, -1.5 * dx, -1.5 * dx, -1.5 * dx,
      -1.5 * dx, -1.5 * dx, -1.5 * dx, -1.5 * dx, 3 * dx,    3 * dx,
      3 * dx,    3 * dx,    3 * dx,    3 * dx,    3 * dx,    3 * dx,
      3 * dx,    -3 * dx,   -3 * dx,   -3 * dx,   -3 * dx,   -3 * dx,
      -3 * dx,   -3 * dx,   -3 * dx,   -3 * dx,   4.5 * dx,  4.5 * dx,
      4.5 * dx,  4.5 * dx,  4.5 * dx,  4.5 * dx,  4.5 * dx,  4.5 * dx,
      -4.5 * dx, -4.5 * dx, -4.5 * dx, -4.5 * dx, -4.5 * dx, -4.5 * dx,
      -4.5 * dx, -4.5 * dx, 6 * dx,    6 * dx,    6 * dx,    6 * dx,
      6 * dx,    6 * dx,    6 * dx,    -6 * dx,   -6 * dx,   -6 * dx,
      -6 * dx,   -6 * dx,   -6 * dx,   -6 * dx,   7.5 * dx,  7.5 * dx,
      7.5 * dx,  7.5 * dx,  7.5 * dx,  7.5 * dx,  -7.5 * dx, -7.5 * dx,
      -7.5 * dx, -7.5 * dx, -7.5 * dx, -7.5 * dx};

  double y[kNMirror] = {
      2 * dy,  4 * dy,  6 * dy,  8 * dy,  -2 * dy, -4 * dy, -6 * dy, -8 * dy,
      1 * dy,  3 * dy,  5 * dy,  7 * dy,  9 * dy,  -1 * dy, -3 * dy, -5 * dy,
      -7 * dy, -9 * dy, 1 * dy,  3 * dy,  5 * dy,  7 * dy,  9 * dy,  -1 * dy,
      -3 * dy, -5 * dy, -7 * dy, -9 * dy, 0 * dy,  2 * dy,  4 * dy,  6 * dy,
      8 * dy,  -2 * dy, -4 * dy, -6 * dy, -8 * dy, 0 * dy,  2 * dy,  4 * dy,
      6 * dy,  8 * dy,  -2 * dy, -4 * dy, -6 * dy, -8 * dy, 1 * dy,  3 * dy,
      5 * dy,  7 * dy,  -1 * dy, -3 * dy, -5 * dy, -7 * dy, 1 * dy,  3 * dy,
      5 * dy,  7 * dy,  -1 * dy, -3 * dy, -5 * dy, -7 * dy, 0 * dy,  2 * dy,
      4 * dy,  6 * dy,  -2 * dy, -4 * dy, -6 * dy, 0 * dy,  2 * dy,  4 * dy,
      6 * dy,  -2 * dy, -4 * dy, -6 * dy, 1 * dy,  3 * dy,  5 * dy,  -1 * dy,
      -3 * dy, -5 * dy, 1 * dy,  3 * dy,  5 * dy,  -1 * dy, -3 * dy, -5 * dy};
  ...
  for (int i = 0; i < kNMirror; i++) {
    ...
    TGeoTranslation* trans =
        new TGeoTranslation(Form("mirTrans%d", i), x[i], y[i], z);
...
void AddCamera(AOpticalComponent* opt) {
  ...
  // Make a camera box
  TGeoBBox* boxCamera = new TGeoBBox("boxCamera", kCameraBoxD / 2.,
                                     kCameraBoxD / 2., kCameraBoxH / 2.);
  double t = 10 * cm;
  TGeoBBox* boxCamera2 =
      new TGeoBBox("boxCamera2", kCameraBoxD / 2. - t, kCameraBoxD / 2. - t,
                   kCameraBoxH / 2. - t);
  ...
  TGeoCompositeShape* boxComposite = new TGeoCompositeShape(
      "boxComposite", "boxCamera:transZ1-boxCamera2:transZ2");
...
void AddMasts(AOpticalComponent* opt) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    AGeoUtil::MakePointToPointTube(Form("mast%d", i), v1, v2, 10 * cm, &tube,
                                   &combi);
...
void RayTrace(AOpticsManager* manager, TCanvas* can3D) {
  ...
  TH2D* hMirror =
      new TH2D("hMirror", ";X (mm);Y (mm)", 1000, -7, 7, 1000, -7, 7);

  for (int i = 0; i < kNdeg; i++) {
    double deg = i * 0.5;
    TGeoTranslation raytr("raytr",
                          -2 * kF * TMath::Sin(deg * TMath::DegToRad()), 0,
                          2 * kF * TMath::Cos(deg * TMath::DegToRad()));
    TVector3 dir;
    dir.SetMagThetaPhi(1, TMath::Pi() - deg * TMath::DegToRad(), 0);
    double lambda = 400 * nm;  // dummy
    ARayArray* array =
        ARayShooter::Square(lambda, 14 * m, 401, 0, &raytr, &dir);

    manager->TraceNonSequential(*array);

    h2[i] = new TH2D("", Form("#it{#theta} = %3.1f#circ;x (mm); y (mm)", deg),
                     200, -40, 100, 200, -70, 70);
