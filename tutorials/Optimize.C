...
double Func(const double* par) {
  ...
  AGeoAsphericDisk* disk = new AGeoAsphericDisk("disk", 0 * mm, 0., par[0],
                                                par[1], kRadius, 0. * mm);
  ...
  AGeoAsphericDisk* disk2 = new AGeoAsphericDisk("disk2", 0 * mm, 0., par[0],
                                                 0., kRadius * 1.2, kRadius);
  ...
  for (int i = 0; i < kNtheta; i++) {
    ...
    gArray[i] =
        ARayShooter::Circle(kLambda, kRadius * 1.1, 20, 10, &rot[i], &tr);
...
void DrawPSF() {
  ...
  for (int i = 0; i < kNtheta; i++) {
    ...
    gPad->DrawFrame(-300, -300, 300, 300,
                    Form("%.1f (deg);X (#it{#mu}m);Y (#it{#mu}m)", kTheta[i]));
...
void Optimize() {
  ...
  min2.SetLimitedVariable(2, "conic constant", par[2], step[2], pmin[2],
                          pmax[2]);
  min2.Minimize();
