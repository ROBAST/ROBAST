...
void SchmidtCassegrain() {
  ...
  AGeoAsphericDisk* disk = new AGeoAsphericDisk(
      "disk", 0. * inch + kOffset, 0 / inch, 0.65 * inch + kOffset,
      -8.721454939626E-005 / inch, 12 * inch, 0. * inch);
  ...
  TGeoCombiTrans* tr1 = new TGeoCombiTrans("tr1", dx * TMath::Cos(90 * deg),
                                           dx * TMath::Sin(90 * deg), 0,
                                           new TGeoRotation("", 90, 0, 0));
  TGeoCombiTrans* tr2 = new TGeoCombiTrans("tr2", dx * TMath::Cos(210 * deg),
                                           dx * TMath::Sin(210 * deg), 0,
                                           new TGeoRotation("", 210, 0, 0));
  TGeoCombiTrans* tr3 = new TGeoCombiTrans("tr3", dx * TMath::Cos(330 * deg),
                                           dx * TMath::Sin(330 * deg), 0,
  ...
  TGeoCompositeShape* comp =
      new TGeoCompositeShape("comp", "box2:tr1 + box2:tr2 + box2:tr3 + tube2");
  obs = new AObscuration("obs", comp);

  top->AddNode(obs, 1,
               new TGeoTranslation(0, 0, (0.65 + 40.0) * inch + kOffset));

  disk = new AGeoAsphericDisk(
      "disk2", (0.65 + 40. + 32.) * inch + kOffset, -1.049567394559E-002 / inch,
      (0.65 + 40. + 32. + 0.1) * inch + kOffset, -1.049567394559E-002 / inch,
      12.183 * inch, 4 * inch);
  ...
  disk = new AGeoAsphericDisk(
      "disk3", (0.65 + 40. + 32. - 30.86635 - 0.1) * inch + kOffset,
      -2.01270013787E-002 / inch,
      (0.65 + 40. + 32. - 30.86635) * inch + kOffset,
      -2.01270013787E-002 / inch, 4.322385947053 * inch, 0);
  mirror = new AMirror("secondary", disk);
  top->AddNode(mirror, 1);

  Double_t origin1[3] = {
      0, 0,
      (0.65 + 40. + 32. - 30.86635 + 50.6706488) * inch + kOffset + 5 * um};
  ...
  for (Int_t j = 0; j < 2; j++) {
    h[j] = new TH2D(Form("h%d", j), Form("%s;X (#mum);Y (#mum);", title[j]),
                    500, -1.25e-3 * inch / um, 1.25e-3 * inch / um, 500,
                    -1.25e-3 * inch / um, 1.25e-3 * inch / um);

    Double_t wave[] = {486.1, 530, 587.6, 610, 656.33};
    for (Int_t k = 0; k < 5; k++) {
      ARayArray* array = ARayShooter::RandomCircle(
          wave[k % 5] * nm, 12.5 * inch, 200000, j == 0 ? 0 : rot);
      ...
      for (Int_t i = 0; i <= focused->GetLast(); i++) {
        ...
        h[j]->Fill(p[0] / um,
                   -(p[1] / um +
                     (j == 0 ? 0. : 0.253015389 * inch / um * angle / 0.1)));
      }  // i
      delete array;
    }  // k
