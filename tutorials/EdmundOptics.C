...
void EdmundOptics() {
  ...
  Double_t coefficients[3] = {0, 4.66252900195e-6 / (mm * mm * mm),
                              -8.02842124899e-9 / (mm * mm * mm * mm * mm)};
  ...
  Double_t wavelength[nColor] = {486.1 * nm, 587.6 * nm,
                                 656.3 * nm};  // n_F, n_d, n_C
  TH2D* spot[nColor];

  for (Int_t j = 0; j < nColor; j++) {
    ARayArray* array =
        ARayShooter::Circle(wavelength[j], rmax * 1.1, 50, 4, rot, tr);

    manager->TraceNonSequential(*array);

    spot[j] = new TH2D(Form("spot%d", j),
                       Form("Spot Diagram for #it{#lambda} = %5.1f (nm);X "
                            "(#it{#mu}m);Y (#it{#mu}m)",
                            wavelength[j] / nm),
                       500, -25, 25, 500, -25, 25);
