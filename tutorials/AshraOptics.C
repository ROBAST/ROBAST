...
void AshraOptics() {
  ...
  const Double_t kSpectrum[kLambdaN][3] = {
      {300 * nm, 0.0810219, 0.0900244},
  ...
  for (Int_t n = 0; n < kN; n++) {
    for (Int_t j = 0; j < 3; j++) {
      hist[n][j] =
          new TH2D(Form("hist%d_%d", n, j), "", 100, -1, 1, 100, -1, 1);
    }  // j

    TGeoRotation* rayrot = new TGeoRotation;
    rayrot->SetAngles(90, 90 - n, 0);
    TGeoTranslation* raytr = new TGeoTranslation(
        "raytr", -500 * mm * TMath::Cos(n * TMath::Pi() / 180.), 0,
        -500 * mm * TMath::Sin(n * TMath::Pi() / 180.));

    for (Int_t i = 0; i < kLambdaN; i++) {
      ARayArray* array = ARayShooter::Rectangle(
          kSpectrum[i][0], 1200 * mm, 1200 * mm, 20, 20, rayrot, raytr);
  ...
  gl->SetPerspectiveCamera(TGLViewer::kCameraPerspXOZ, 25., 0, 0,
                           -75 * TMath::DegToRad(), 210. * TMath::DegToRad());
  ...
  for (Int_t i = 0; i < 3; i++) {
    ...
    for (Int_t j = 0; j < kN; j++) {
      Double_t val = TMath::Sqrt(
          TMath::Power(hist[j][i]->GetRMS(1), 2) +
          TMath::Power(
              hist[j][i]->GetRMS(2) / TMath::Cos(TMath::Pi() * j / 180), 2));
...
void AddMirror(AOpticalComponent* opt) {
  ...
  const Double_t kMirrorR[kMirrorN] = {1363 * mm, 1363 * mm, 1363 * mm,
  ...
  for (Int_t i = 0; i < kMirrorN; i++) {
    ...
    if (i == 0) {
      ...
      cut1->DefineSection(0, 0, 0,
                          1 * nm);  // 1*nm is a hack to avoid OpenGL bug
      ...
      for (Int_t j = 0; j < 2; j++) {
        ...
        if (j == 0)
          ...
          r_ = r;  // hack again
        cut2[i]->SetVertex(0 + j * 4, r_ * TMath::Cos(60 * deg),
                           r_ * TMath::Sin(60 * deg));
        cut2[i]->SetVertex(1 + j * 4, r_ * TMath::Cos(-8.6 * deg),
                           r_ * TMath::Sin(-8.6 * deg));
        cut2[i]->SetVertex(2 + j * 4, r_ * TMath::Cos(188.6 * deg),
                           r_ * TMath::Sin(188.6 * deg));
        cut2[i]->SetVertex(3 + j * 4, r_ * TMath::Cos(120 * deg),
                           r_ * TMath::Sin(120 * deg));

        cut3[i]->SetVertex(0 + j * 4, r_ * TMath::Cos(351.4 * deg),
                           r_ * TMath::Sin(351.4 * deg));
        cut3[i]->SetVertex(1 + j * 4, 0, r_ / r * -10000 * mm);
        cut3[i]->SetVertex(2 + j * 4, r_ * TMath::Cos(188.6 * deg),
                           r_ * TMath::Sin(188.6 * deg));
    ...
    sphere[i] = new TGeoSphere(Form("mir_sph%d", i), R, R_, 0,
                               TMath::ASin(kMirrorRad / R) / deg);
    if (i == 0) {
      composite[i] = new TGeoCompositeShape(
          Form("mir_cs%d", i), Form("(mir_sph%d*mir_cut1):mir_tr%d", i, i));
    } else {
      composite[i] = new TGeoCompositeShape(
          Form("mir_cs%d", i),
          Form("mir_sph%d*((mir_cut2%d + mir_cut3%d):mir_tr%d)", i, i, i, i));
    }  // if
...
void AddLens(AOpticalComponent* opt) {
  ...
  const Double_t kLensRadius[3][2] = {
      {590 * mm, 85 * mm}, {500 * mm, 140 * mm}, {590 * mm, 205 * mm}};
  const Double_t kLensPol[3][4] = {
      {0, 2.50657e-10 * TMath::Power(mm, -3),
       -3.26591e-16 * TMath::Power(mm, -5),
       -2.30099e-22 * TMath::Power(mm, -7)},
      {0, -3.16481e-10 * TMath::Power(mm, -3),
       6.23315e-16 * TMath::Power(mm, -5), 8.71537e-22 * TMath::Power(mm, -7)},
      {0, 2.48098e-10 * TMath::Power(mm, -3),
       -3.44020e-16 * TMath::Power(mm, -5),
       -1.99402e-22 * TMath::Power(mm, -7)}};

  // Acrylite data measured with V-Block method
  ASchottFormula* acrylite =
      new ASchottFormula(+2.192728e-0, -5.320698e-3, +7.215869e-3, +1.657987e-3,
  ...
  for (Int_t i = 0; i < 3; i++) {
    a[i] = new AGeoAsphericDisk(Form("lens_a%d", i + 1), kLensZ[i][0], 0,
                                kLensZ[i][1], 1 / kLensR[i], kLensRadius[i][0],
                                kLensRadius[i][1]);
  ...
  TGeoCompositeShape* composite1 =
      new TGeoCompositeShape("lens_cs1", "lens_box1-lens_tube1");
  TGeoCompositeShape* composite2 =
      new TGeoCompositeShape("lens_cs2", "lens_box1-lens_tube2");
...
void AddPipeline(AOpticalComponent* opt) {
  ...
  const Double_t kHaubeZ[kHaubeN] = {
      0 * mm,   4 * mm,   4 * mm,   194 * mm, 194 * mm, 202 * mm, 202 * mm,
      298 * mm, 613 * mm, 717 * mm, 717 * mm, 720 * mm, 720 * mm, 730 * mm};
  const Double_t kHaubeRmax[kHaubeN] = {
      320 * mm, 320 * mm, 315 * mm, 315 * mm, 315 * mm, 315 * mm, 291 * mm,
      291 * mm, 119 * mm, 119 * mm, 119 * mm, 119 * mm, 93 * mm,  93 * mm};
  const Double_t kHaubeRmin[kHaubeN] = {
      310 * mm, 310 * mm, 310 * mm, 310 * mm, 287 * mm, 287 * mm, 287 * mm,
      287 * mm, 115 * mm, 115 * mm, 0 * mm,   0 * mm,   0 * mm,   0 * mm};
  ...
  */
  ...
  TGeoTranslation* tr1 =
      new TGeoTranslation("pip_tr1", 15 * mm, -6.7987 * mm, 0);
  tr1->RegisterYourself();
  TGeoTranslation* tr2 =
      new TGeoTranslation("pip_tr2", -15 * mm, -6.7987 * mm, 0);
  ...
  TGeoCompositeShape* composite1 = new TGeoCompositeShape(
      "pip_cs1",
      "pip_box1-(pip_arb1+pip_arb2+pip_sph1:pip_tr1+pip_sph1:pip_tr2)");
  TGeoCompositeShape* composite2 = new TGeoCompositeShape(
      "pip_cs2",
      "(pip_box1-(pip_arb1+pip_arb3))-(pip_sph1:pip_tr1+pip_sph1:pip_tr3)");
  TGeoCompositeShape* composite3 = new TGeoCompositeShape(
      "pip_cs3",
      "(pip_box1-(pip_arb2+pip_arb4))-(pip_sph1:pip_tr2+pip_sph1:pip_tr4)");
  ...
  opt->AddNode(Buhin1_1, 1,
               new TGeoCombiTrans(0, 309 * mm, kOffset - 212 * mm,
                                  new TGeoRotation("pip_rot1", 180, 0, 0)));
  opt->AddNode(Buhin1_2, 1,
               new TGeoCombiTrans(-309 * mm * sqrt(3) / 2, -309 * mm / 2,
                                  kOffset - 212 * mm,
                                  new TGeoRotation("pip_rot2", -60, 0, 0)));
  opt->AddNode(Buhin1_3, 1,
               new TGeoCombiTrans(309 * mm * sqrt(3) / 2, -309 * mm / 2,
                                  kOffset - 212 * mm,
                                  new TGeoRotation("pip_rot3", 60, 0, 0)));
}

//_____________________________________________________________________________
void AddInputWindow(AOpticalComponent* opt) {
  // Kovar glass
  ASchottFormula* kovar =
      new ASchottFormula(+2.176400e-0, -5.051050e-3, +1.379077e-2, -9.428276e-4,
                         +1.304423e-4, -5.956672e-6);

  // Input window of 20" II
  TGeoSphere* sphere1 =
      new TGeoSphere("in_sphere1", kInputR[0], kInputR[1], 0, 24);
  ...
  // Focal plane
  TGeoSphere* sphere2 =
      new TGeoSphere("in_sphere2", kInputR[0] - .1 * mm, kInputR[0], 0, 24);
...
void AddLensRing(AOpticalComponent* opt) {
  // LR-003
  TGeoTube* tube1 =
      new TGeoTube("lring_tube1", 597.5 * mm, 599.5 * mm, 19 * mm);
  ...
  TGeoCompositeShape* composite1 =
      new TGeoCompositeShape("lring_cs1", "lring_tube1+lring_tube2:lring_tr1");
  ...
  // LR-001
  TGeoTube* tube3 =
      new TGeoTube("lring_tube3", 597.5 * mm, 599.5 * mm, 81.5 * mm);
  ...
  TGeoCompositeShape* composite2 = new TGeoCompositeShape(
      "lring_cs2",
      "(lring_tube3+lring_tube2:lring_tr2+lring_tube2:lring_tr3)-(lring_tube4:"
      "lring_rot2+lring_tube4:lring_rot3)");
...
void AddLensFrame(AOpticalComponent* opt) {
  // LF-150
  TGeoBBox* box1 =
      new TGeoBBox("lframe_box1", 662.5 * mm, 662.5 * mm, 300 * mm);
  TGeoBBox* box2 =
      new TGeoBBox("lframe_box2", 657.5 * mm, 657.5 * mm, 295 * mm);
  TGeoBBox* box3 =
      new TGeoBBox("lframe_box3", 662.6 * mm, 612.5 * mm, 250 * mm);
  TGeoBBox* box4 =
      new TGeoBBox("lframe_box4", 612.5 * mm, 662.6 * mm, 250 * mm);
  TGeoBBox* box5 =
      new TGeoBBox("lframe_box5", 612.6 * mm, 612.6 * mm, 300.1 * mm);
  TGeoCompositeShape* composite1 = new TGeoCompositeShape(
      "lframe_cs1",
      "lframe_box1-(lframe_box2+lframe_box3+lframe_box4+lframe_box5)");
  ...
  TGeoBBox* box8 =
      new TGeoBBox("lframe_box8", 340 / r2 * mm, 340 / r2 * mm, 30 * mm);
  TGeoTranslation* tr1 = new TGeoTranslation("lframe_tr1", 0, 0, 2.5 * mm);
  tr1->RegisterYourself();
  TGeoTranslation* tr2 =
      new TGeoTranslation("lframe_tr2", 50 / r2 * mm, 50 / r2 * mm, 0);
  tr2->RegisterYourself();
  TGeoRotation* rot1 = new TGeoRotation("lframe_rot1", -45, 0, 0);
  rot1->RegisterYourself();
  TGeoCompositeShape* composite2 =
      new TGeoCompositeShape("lframe_cs2",
                             "((lframe_box6-lframe_box7:lframe_tr1):lframe_"
                             "rot1)*(lframe_box8:lframe_tr2)");
  ...
  opt->AddNode(LF151, 1,
               new TGeoCombiTrans(-(657.5 - 145 * r2) * mm,
                                  -(657.5 - 145 * r2) * mm, 295 * mm, 0));
  opt->AddNode(LF151, 2,
               new TGeoCombiTrans(+(657.5 - 145 * r2) * mm,
                                  -(657.5 - 145 * r2) * mm, 295 * mm, rot2));
  opt->AddNode(LF151, 3,
               new TGeoCombiTrans(+(657.5 - 145 * r2) * mm,
                                  +(657.5 - 145 * r2) * mm, 295 * mm, rot3));
  opt->AddNode(LF151, 4,
               new TGeoCombiTrans(-(657.5 - 145 * r2) * mm,
                                  +(657.5 - 145 * r2) * mm, 295 * mm, rot4));
  ...
  opt->AddNode(LF151, 5,
               new TGeoCombiTrans(-(657.5 - 145 * r2) * mm,
                                  -(657.5 - 145 * r2) * mm, -300 * mm, rot5));
  opt->AddNode(LF151, 6,
               new TGeoCombiTrans(+(657.5 - 145 * r2) * mm,
                                  -(657.5 - 145 * r2) * mm, -300 * mm, rot6));
  opt->AddNode(LF151, 7,
               new TGeoCombiTrans(+(657.5 - 145 * r2) * mm,
                                  +(657.5 - 145 * r2) * mm, -300 * mm, rot7));
  opt->AddNode(LF151, 8,
               new TGeoCombiTrans(-(657.5 - 145 * r2) * mm,
                                  +(657.5 - 145 * r2) * mm, -300 * mm, rot8));
}
...
void AddStewart(AOpticalComponent* opt) {
  ...
  tr1->RegisterYourself();
  TGeoCompositeShape* composite1 =
      new TGeoCompositeShape("st_cs1", "st_tube1+st_pgon1:st_tr1");
  ...
  tr2->RegisterYourself();
  TGeoCompositeShape* composite2 =
      new TGeoCompositeShape("st_cs2", "st_tube2+st_pgon1:st_tr2");
  ...
  TGeoCompositeShape* composite3 =
      new TGeoCompositeShape("st_cs3",
                             "st_pcon1:st_tr3+st_pcon1:st_rot1+st_pgon2:st_tr4+"
                             "st_tube2:st_tr5+st_pgon1:st_tr6+st_pgon1:st_tr7");
  AObscuration* BE = new AObscuration("BE", composite3);
  BE->SetLineColor(16);
  opt->AddNode(BE, 1,
               new TGeoCombiTrans(-550 * mm, 303 * mm, 348.5 * mm,
                                  new TGeoRotation("st_rot2", 32, -104.8, 0)));
  opt->AddNode(BE, 2,
               new TGeoCombiTrans(550 * mm, 303 * mm, 348.5 * mm,
                                  new TGeoRotation("st_rot3", -32, -104.8, 0)));
  ...
  TGeoCompositeShape* composite4 = new TGeoCompositeShape(
      "st_cs4",
      "st_pcon1:st_tr8+st_pcon1:st_rot1+st_pgon2:st_tr4+st_tube1:st_tr9+st_"
      "pgon1:st_tr10+st_pgon1:st_tr11");
  AObscuration* CD = new AObscuration("CD", composite4);
  CD->SetLineColor(16);
  opt->AddNode(CD, 1,
               new TGeoCombiTrans(-304 * mm, 551 * mm, 349 * mm,
                                  new TGeoRotation("st_rot4", 51, -109.5, 0)));
  opt->AddNode(CD, 2,
               new TGeoCombiTrans(304 * mm, 551 * mm, 349 * mm,
                                  new TGeoRotation("st_rot5", -51, -109.5, 0)));
  ...
  TGeoCompositeShape* composite5 = new TGeoCompositeShape(
      "st_cs5",
      "st_pcon1:st_tr12+st_pcon1:st_rot1+st_pgon2:st_tr4+st_tube1:st_tr13+st_"
      "pgon1:st_tr14+st_pgon1:st_tr15");
  AObscuration* AF = new AObscuration("AF", composite5);
  AF->SetLineColor(16);
  opt->AddNode(AF, 1,
               new TGeoCombiTrans(-334 * mm, -530 * mm, 350 * mm,
                                  new TGeoRotation("st_rot6", -11, 110, 0)));
  opt->AddNode(AF, 2,
               new TGeoCombiTrans(334 * mm, -530 * mm, 350 * mm,
                                  new TGeoRotation("st_rot7", 11, 110, 0)));
  ...
  TGeoTranslation* tr17 = new TGeoTranslation(
      "st_tr17", (14.5 + 15 / sqrt(30 * 30 + 15.5 * 15.5) * 15.5) * mm, 0,
      (40 - 15 / sqrt(30 * 30 + 15.5 * 15.5) * 30) * mm);
  tr17->RegisterYourself();
  TGeoCompositeShape* composite6 = new TGeoCompositeShape(
      "st_cs6", "st_tube3-(st_arb1:st_tr16 + st_sph1:st_tr17)");
  ...
  opt->AddNode(
      SW003, 1,
      new TGeoCombiTrans(x1, y1, z, new TGeoRotation("st_rot1", -135, 0, 0)));
  opt->AddNode(
      SW003, 2,
      new TGeoCombiTrans(-x1, y1, z, new TGeoRotation("st_rot2", -45, 0, 0)));
  opt->AddNode(
      SW003, 3,
      new TGeoCombiTrans(x2, y2, z, new TGeoRotation("st_rot3", -135, 0, 0)));
  opt->AddNode(
      SW003, 4,
      new TGeoCombiTrans(-x2, y2, z, new TGeoRotation("st_rot4", -45, 0, 0)));
  opt->AddNode(
      SW003, 5,
      new TGeoCombiTrans(x3, y3, z, new TGeoRotation("st_rot5", 90, 0, 0)));
  opt->AddNode(
      SW003, 6,
      new TGeoCombiTrans(-x3, y3, z, new TGeoRotation("st_rot6", 90, 0, 0)));
}
...
void Add30Frame(AOpticalComponent* opt) {
  ...
  tr2->RegisterYourself();
  TGeoCompositeShape* composite1 = new TGeoCompositeShape(
      "30_cs1", "30_xtru1+30_box1:30_tr1+30_box1:30_tr2");
  ...
  TGeoTranslation* tr3 =
      new TGeoTranslation("30_tr3", 54.5 * mm, 0, 662.5 * mm);
  tr3->RegisterYourself();
  TGeoTranslation* tr4 =
      new TGeoTranslation("30_tr4", 54.5 * mm, 0, -662.5 * mm);
  tr4->RegisterYourself();
  ...
  TGeoTranslation* tr5 =
      new TGeoTranslation("30_tr5", 0, 0, (739.5 + 50 / r3) * mm);
  tr5->RegisterYourself();
  TGeoTranslation* tr6 =
      new TGeoTranslation("30_tr6", 0, 0, (-1482.5 + 50 / r3) * mm);
  tr6->RegisterYourself();
  TGeoRotation* rot2 = new TGeoRotation("30_rot2", 0, -30, 0);
  TGeoCombiTrans* cm1 =
      new TGeoCombiTrans("30_cm1", 0, (-150 * r3 / 2 + 50 + 4.5 / 2) * mm,
                         (739.5 + 150 / 2. + 4.5 * r3 / 2) * mm, rot2);
  cm1->RegisterYourself();
  TGeoCombiTrans* cm2 =
      new TGeoCombiTrans("30_cm2", 0, (-145 * r3 / 2 + 50 - 4.5 / 2) * mm,
                         (-1482.5 + 145 / 2. - 4.5 * r3 / 2) * mm, rot2);
  cm2->RegisterYourself();

  TGeoCompositeShape* composite2 = new TGeoCompositeShape(
      "30_cs2",
      "((30_xtru2+30_box2:30_tr3+30_box2:30_tr4)-(30_arb1:30_tr5+30_arb2:30_"
      "tr6))+30_box3:30_cm1+30_box3:30_cm2");
  ...
  TGeoTranslation* tr7 =
      new TGeoTranslation("30_tr7", -54.5 * mm, 0, 662.5 * mm);
  tr7->RegisterYourself();
  TGeoTranslation* tr8 =
      new TGeoTranslation("30_tr8", -54.5 * mm, 0, -662.5 * mm);
  tr8->RegisterYourself();
  TGeoCompositeShape* composite3 = new TGeoCompositeShape(
      "30_cs3",
      "((30_xtru2+30_box2:30_tr7+30_box2:30_tr8)-(30_arb1:30_tr5+30_arb2:30_"
      "tr6))+30_box3:30_cm1+30_box3:30_cm2");
  ...
  tr10->RegisterYourself();
  TGeoCompositeShape* composite4 = new TGeoCompositeShape(
      "30_cs4", "30_xtru3+30_box1:30_tr9+30_box1:30_tr10");
  AObscuration* F30_001 = new AObscuration("F30_001", composite4);

  TGeoRotation* rot4 = new TGeoRotation("30_rot4", 90, 90, 90);
  comp->AddNode(F30_001, 1,
                new TGeoCombiTrans(0, -2050 * mm, -1116 * mm, rot1));
  ...
  tr11->RegisterYourself();
  TGeoCombiTrans* cm3 =
      new TGeoCombiTrans("30_cm3", 54.5 * mm, 0, -1010.5 * mm, rot1);
  cm3->RegisterYourself();
  TGeoCompositeShape* composite5 = new TGeoCompositeShape(
      "30_cs5", "30_xtru4+30_box1:30_tr11+30_box3:30_cm3");
  AObscuration* F30_009 = new AObscuration("F30_009", composite5);
  TGeoRotation* rot5 = new TGeoRotation("30_rot5", 90, 0, 0);
  comp->AddNode(F30_009, 1,
                new TGeoCombiTrans(662.5 * mm, -2050 * mm, -5.5 * mm, rot5));
  comp->AddNode(F30_009, 2,
                new TGeoCombiTrans(-662.5 * mm, -2050 * mm, -5.5 * mm, rot5));
  ...
  tr12->RegisterYourself();
  TGeoCompositeShape* composite6 =
      new TGeoCompositeShape("30_cs6", "30_xtru5+30_box1:30_tr12");
  ...
  TGeoCombiTrans* cm4 =
      new TGeoCombiTrans("30_cm4", 54.5 * mm, 0, 663 * mm, rot4);
  cm4->RegisterYourself();
  TGeoCombiTrans* cm5 =
      new TGeoCombiTrans("30_cm5", 54.5 * mm, 0, -1025 * mm, rot4);
  cm5->RegisterYourself();

  TGeoCompositeShape* composite7 = new TGeoCompositeShape(
      "30_cs7", "30_xtru6+30_box1:30_cm4+30_box1:30_cm5");
  AObscuration* F30_010 = new AObscuration("F30_010", composite7);
  TGeoRotation* rot6 = new TGeoRotation("30_rot6", 180, 90, 0);
  comp->AddNode(F30_010, 1,
                new TGeoCombiTrans(-662.5 * mm, -1025 * mm, 1214 * mm, rot3));
  comp->AddNode(F30_010, 2,
                new TGeoCombiTrans(662.5 * mm, -1025 * mm, 1214 * mm, rot6));
  ...
  TGeoTranslation* tr13 =
      new TGeoTranslation("30_tr13", 0, 0, (1109 - 68.5 * r3 / 2) * mm);
  tr13->RegisterYourself();
  TGeoTranslation* tr14 =
      new TGeoTranslation("30_tr14", 0, 0, (-814 + 68.5 / 2 / 2) * mm);
  tr14->RegisterYourself();
  TGeoTranslation* tr15 =
      new TGeoTranslation("30_tr15", 0, 54.5 * mm, 533 * mm);
  tr15->RegisterYourself();
  TGeoTranslation* tr16 =
      new TGeoTranslation("30_tr16", 0, 54.5 * mm, -533 * mm);
  tr16->RegisterYourself();

  TGeoCompositeShape* composite8 = new TGeoCompositeShape(
      "30_cs8", "30_xtru7-(30_arb3:30_tr13+30_arb4:30_tr14)");
  ...
  TGeoTranslation* tr17 =
      new TGeoTranslation("30_tr17", 0, 0, (-267.5 + 75 * r3 / 2) * mm);
  tr17->RegisterYourself();

  TGeoRotation* rot7 = new TGeoRotation("30_rot7", 0, -120, 0);
  TGeoCombiTrans* cm6 = new TGeoCombiTrans(
      "30_cm6", 0, (-75 + 267.5 * r3 / 2 + 75 / 2. / 2.) * mm,
      (-4.5 - 267.5 / 2 + 75 * r3 / 2 / 2) * mm - 1 * nm, rot7);
  cm6->RegisterYourself();

  TGeoBBox* box6 = new TGeoBBox("30_box6", 150 * mm, 5 * mm, 150 * mm);
  TGeoTranslation* tr18 = new TGeoTranslation(
      "30_tr18", 0, (37.5 + 5) * mm + 1 * nm, (267.5 - 150) * mm);
  tr18->RegisterYourself();
  TGeoTranslation* tr19 = new TGeoTranslation(
      "30_tr19", 0, (37.5 + 5 + 100) * mm, (267.5 - 150) * mm);
  tr19->RegisterYourself();

  TGeoCompositeShape* composite9 = new TGeoCompositeShape(
      "30_cs9", "(30_xtru8-30_arb5:30_tr17)+30_box6:30_tr18+30_box6:30_tr19");
  TGeoCompositeShape* composite10 =
      new TGeoCompositeShape("30_cs10", "(30_box5:30_rot3)+(30_cs9:30_cm6)");
  ...
  TGeoCombiTrans* cm7 =
      new TGeoCombiTrans("30_cm7", 0, (37.5 + 4.5) * mm, 533 * mm, rot8);
  cm7->RegisterYourself();
  TGeoCombiTrans* cm8 =
      new TGeoCombiTrans("30_cm8", 0, (37.5 + 4.5) * mm, -533 * mm, rot9);
  cm8->RegisterYourself();
  TGeoCombiTrans* cm9 =
      new TGeoCombiTrans("30_cm9", 0, (37.5 + 4.5) * mm, 533. / 2 * mm, rot10);
  cm9->RegisterYourself();
  TGeoCombiTrans* cm10 = new TGeoCombiTrans("30_cm10", 0, (37.5 + 4.5) * mm,
                                            -533. / 2 * mm, rot11);
  cm10->RegisterYourself();
  TGeoCombiTrans* cm11 =
      new TGeoCombiTrans("30_cm11", 0, (37.5 + 4.5) * mm, 533. / 2 * mm, rot12);
  cm11->RegisterYourself();
  TGeoCombiTrans* cm12 = new TGeoCombiTrans("30_cm12", 0, (37.5 + 4.5) * mm,
                                            -533. / 2 * mm, rot13);
  cm12->RegisterYourself();

  TGeoCompositeShape* composite11 = new TGeoCompositeShape(
      "30_cs11", "30_cs8+(30_cs10:30_cm7)+(30_cs10:30_cm8)");
  TGeoCompositeShape* composite12 = new TGeoCompositeShape(
      "30_cs12", "30_cs8+(30_cs10:30_cm11)+(30_cs10:30_cm12)");
  TGeoCompositeShape* composite13 = new TGeoCompositeShape(
      "30_cs13", "30_cs8+(30_cs10:30_cm9)+(30_cs10:30_cm10)");
  ...
  comp->AddNode(
      F30_0064, 1,
      new TGeoCombiTrans(
          0 * mm, (-363 + 50 - 15 + 6.5 / 2 - 1109 * r3 / 2. - 37.5 / 2) * mm,
          (1214 - 50 - 9 - 1109 / 2 - 6.5 / 2 * r3 + 37.5 * r3 / 2) * mm,
          rot14));
  comp->AddNode(
      F30_0054a, 1,
      new TGeoCombiTrans(
          462 * mm, (-363 + 50 - 15 + 6.5 / 2 - 1109 * r3 / 2. - 37.5 / 2) * mm,
          (1214 - 50 - 9 - 1109 / 2 - 6.5 / 2 * r3 + 37.5 * r3 / 2) * mm,
          rot14));
  comp->AddNode(
      F30_0054b, 1,
      new TGeoCombiTrans(
          -462 * mm,
          (-363 + 50 - 15 + 6.5 / 2 - 1109 * r3 / 2. - 37.5 / 2) * mm,
          (1214 - 50 - 9 - 1109 / 2 - 6.5 / 2 * r3 + 37.5 * r3 / 2) * mm,
          rot14));

  opt->AddNodeOverlap(
      comp, 1,
      new TGeoCombiTrans(0, (739.5 + 50 / 2. + (9 + 50) * r3 / 2) * mm,
                         (325 - (50 + 9) / 2. + 50 * r3 / 2) * mm, rot2));
}
